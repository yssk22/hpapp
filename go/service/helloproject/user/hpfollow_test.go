...
func TestHPFollow(t *testing.T) {
	test.New(
		...
		juice := entclient.HPArtist.Query().Where(hpartist.KeyEQ("juicejuice")).FirstX(ctx)
	...
	test.New(
		...
		entclient := entutil.NewClient(ctx)
		mizuki := entclient.HPMember.Query().Where(hpmember.KeyEQ("mizuki_fukumura")).FirstX(ctx)
		morning := entclient.HPArtist.Query().Where(hpartist.KeyEQ("morningmusume")).FirstX(ctx)
		myId := appuser.CurrentEntUserID(ctx)
		...
		a.Nil(err)
		a.Equals(1, morning.QueryFollowedBy().CountX(ctx))
		follow = morning.QueryFollowedBy().WithUser().FirstX(ctx)
		a.Equals(enums.HPFollowTypeFollowWithNotification, follow.Type)
		a.Equals(myId, follow.Edges.User.ID)
