...
func (tx *Tx) init() {
	...
	tx.HPAmebloPost = NewHPAmebloPostClient(tx.config)
	tx.HPArtist = NewHPArtistClient(tx.config)
	...
	tx.HPFollow = NewHPFollowClient(tx.config)
	tx.HPIgPost = NewHPIgPostClient(tx.config)
	...
	tx.UserNotificationSetting = NewUserNotificationSettingClient(tx.config)
}
