...
type AuthQuery struct {
	...
	loadTotal  []func(context.Context, []*Auth) error
...
func (aq *AuthQuery) Clone() *AuthQuery {
	...
	return &AuthQuery{
		config:     aq.config,
		ctx:        aq.ctx.Clone(),
		order:      append([]auth.Order{}, aq.order...),
...
func (aq *AuthQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(auth.Table, auth.Columns, sqlgraph.NewFieldSpec(auth.FieldID, field.TypeInt))
	_spec.From = aq.sql
	...
	if fields := aq.ctx.Fields; len(fields) > 0 {
		...
		if aq.withUser != nil {
			_spec.Node.AddColumnOnce(auth.FieldOwnerUserID)
		}
	}
