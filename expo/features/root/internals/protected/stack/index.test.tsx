...
test('stack', async () => {
  ...
  const rendered = await renderWithRoot(
  ...
  const buttonToScreenWithoutParams = await rendered.findByTestId('btnNavigateToScreenWithoutParams');
  ...
  expect(text).toBeTruthy();
  const buttonToScreenWithParams = await rendered.findByTestId('btnNavigateToScreenWithParams');
