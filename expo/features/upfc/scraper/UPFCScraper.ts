...
export default class UPFCScraper implements Scraper {
  ...
  async getEventApplications(): Promise<EventApplicationTickets[]> {
    ...
    const [eventsHtml, execEventHtml, ticketsHtml] = await Promise.all([
      events,
      exeEvents,
      tickets,
    ]);
  ...
  parseEventApplications(
    ...
    const execEventApplications =
      this.parseEventApplicationsHtml(execEventHtml);
    const ticketApplications = this.parseTicketsHtml(ticketsHtml);
    const result: EventApplicationTickets[] = [];
    const eventMap: Map<string, EventApplicationTickets> = new Map();
    const allApplications = eventApplications.concat(execEventApplications);
    for (const i in allApplications) {
      const a = allApplications[i];
    ...
    for (const i in ticketApplications) {
      const eat = ticketApplications[i];
  ...
  parseEventApplicationsHtml(text: string): EventApplication[] {
    const doc = jsdom(text);
  ...
  parseTicketsHtml(text: string): EventApplicationTickets[] {
    const doc = jsdom(text);
    ...
    const allApplications: Array<EventApplicationTickets> = [];
    ...
    for (let i = 0; i < eventNameDOMs.length; i++) {
      ...
      const application: EventApplicationTickets = {
      ...
      for (let j = 0; j < rows.length; j++) {
        ...
        if (cols.length === 7) {
          const eventDate = this.parseEventDate(
            cols.item(0).innerHTML,
            cols.item(2).innerHTML.trim()
          );
          const venue = cols
            .item(1)
          ...
          if (status === "入金待") {
            if (!application.paymentDueDate) {
              if (
                eventDate !== null &&
                today.getTime() >= eventDate.startAt.getTime()
              ) {
                status = "入金忘";
              }
            } else {
  ...
  parseEventApplicationLink(link: any): EventApplication | null {
    ...
    if (
      href.match(/event_Event_Announce\.php/) === null &&
      href.match(/fanticket_Tour_List\.php/) === null
    ) {
    ...
    const name = this.parseEventApplicationName(
      span.textContent.trim() as string
    );
  ...
  private parseTicketStatus(valueStr: string): TicketStatus {
  ...
  private extractJPDates(text: string): [Date, Date] | null {
    const matches = text.match(UPFCScraper.jpDateRegexp);
